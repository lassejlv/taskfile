...
async fn main() {
    let matches = Command::new("taskfile")
        ...
        .arg(
            Arg::new("command")
                .help("The command to run (list or task name)")
                .value_name("COMMAND")
                .index(1),
        )
        .get_matches();
            ...
            match TaskRunner::from_file(taskfile_name).await {
                Ok(runner) => {
                    ...
                    match matches.get_one::<String>("command") {
                        ...
                        None => {
                            println!(
                                "Please specify a task to run or use 'list' to see available tasks."
                            );
                            println!("Usage: cargo run <task_name> or cargo run list");
                            std::process::exit(1);
                        }
                    }
                }
